<template>
  <el-dialog
    ...
    top="5vh"
    :modal="true"
    custom-class="EditCommodityDialog"
    :append-to-body="true"
  >
    <div class="dialog_body">
      <div v-if="visible" class="main_dialog">
        <div>
          <h5 class="title">
            基本信息
          </h5>
          <el-form
            ...
            size="small"
            :model="ruleForm"
            :disabled="editGoodsActionCount > 0 || getDetailByIdCount > 0"
            :rules="rules"
            status-icon
            label-width="180px"
          ...
          >
            ...
            <div v-else class="has_option">
              <el-form-item title="详细规格配置表" label="详细规格配置表">
                ...
                <StanderdTable
                  ref="StanderdTable"
                  :init-data="standerdTableFormData"
                  :disabled="editGoodsActionCount > 0 || getDetailByIdCount > 0"
                />
              </el-form-item>
            </div>
    ...
    <div slot="footer" class="dialog_footer">
      <el-button
        :loading="editGoodsActionCount > 0"
        :disabled="getDetailByIdCount > 0"
        size="small"
        @click="onClickSubmit"
      >
        提交
...
export default {
  ...
  computed: {
    ...
    ...mapState('loading', {
      editGoodsActionCount: 'goodsModule/editGoods',
      getDetailByIdCount: 'goodsModule/getDetailById'
    })
  },
  ...
  methods: {
    ...
    async setBaseData() {
      ...
      if (this.isSuccessful(code)) {
        mapValues(omit(data, ['standerdTableFormData']), (value, key) => {
          this.ruleForm[key] = value
        })

        this.$refs.multiSelect && this.$refs.multiSelect.setInitPath(this.ruleForm.tcate)

        this.standerdTableFormData = data.standerdTableFormData
        // this.$refs.StanderdTable.setFormData(data.standerdTableFormData)
      }
    },
    onEditorBlur(editor) {
    ...
    async init() {
      ...
      await this.getDispatchData()
      await this.getAllGoodsCateListData()
      await this.setBaseData()
    },
    // 退出表格
    beforeClose(done) {
      if (this.editGoodsActionCount > 0) {
        return
      }
    ...
    async submit() {
      ...
      const { code, message } = await this.editGoodsAction({
        id: this.dispatchId,
        ...this.ruleForm,
        standerdTableFormData
      })
