<template>
  <div class="AddONewTrademark">
    ...
    <el-dialog
      ...
      custom-class="AddOrEditTrademarkDialog"
      :fullscreen="false"
      :append-to-body="true"
...
export default {
  ...
  computed: {
    ...
    loading() {
      return this.addNewTrademarkCount > 0
    }
  },
  methods: {
    ...mapActions('trademarkModule', ['addNewTrademark']),
    // 回复弹窗关闭前的拦截器
    handleClose(done) {
      done()
    },
    ...
    async submit() {
      const result = await this.addNewTrademark(this.ruleForm)
